...
def evaluate_model(model, X_train, X_test, y_train, y_test, model_name, split_type):
    ...
    # Plot actual data points by label
    unique_labels = sorted(np.unique(y_test))
    x_test_array = X_test['tone'].values
    y_test_array = y_test.values
    
    for label in unique_labels:
        mask = y_test_array == label
        plt.scatter(x_test_array[mask], 
                    [label] * np.sum(mask), 
...
plt.figure(figsize=(8, 6))
rf_importance_time = rf_model.feature_importances_[0] if hasattr(rf_model, 'feature_importances_') else 0
feature_importance = pd.DataFrame(
    {'feature': ['tone'], 
     'importance': [rf_importance_time]}
)
feature_importance.plot(x='feature', y='importance', kind='bar', figsize=(8, 6))
